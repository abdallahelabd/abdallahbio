...
export default function AdminChatPage() {
  ...
  const messagesForUser = chatLog.filter(m =>
    m.userName === selectedUser || m.recipient === selectedUser
  );
    ...
    <div className="h-screen w-screen flex bg-gray-950 text-green-100">
      <aside className="w-1/3 border-r border-green-800 p-4 overflow-y-auto">
        ...
        {users.map(user => (
          <div
            key={user}
            className={\`p-2 rounded-lg cursor-pointer mb-2 \${selectedUser === user ? 'bg-green-700' : 'hover:bg-green-800'}\`}
      ...
      <main className="flex-1 p-4 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto space-y-3">
          {messagesForUser.map((msg) => (
            <div
              key={msg.id}
              className={\`p-3 max-w-[70%] rounded-2xl \${msg.userName === "Abdallah" ? 'bg-green-800 self-end text-right' : 'bg-green-600 self-start text-left'}\`}
            >
              <div>{msg.user}</div>
